...
with closing(psycopg2.connect(dbname='srx_dev', user='srx_dev',
    ...
    with conn.cursor(cursor_factory=DictCursor) as cursor:
        customers_list = list()
        cursor.execute("select ship_to.id from srx.customer left join srx.warehouse on customer.warehouse_id = warehouse.id left join srx.ship_to on customer.id = ship_to.customer_id where warehouse.distributor_id = 2 and customer.created_by = 'LOAD SETUP'")
        fetched_shipto_ids = cursor.fetchall()
        shipto_ids = get_list(fetched_shipto_ids)


        customer_settings_list = list()
        for shipto_id in shipto_ids:
            for _ in range(3):
                customer_setting = {
                    "created_at": now(),
        ...
        cursor.execute("select general_setting.id, general_setting.ship_to_id from  srx.ship_to left join srx.general_setting on ship_to.id = general_setting.ship_to_id left join srx.customer on customer.id = ship_to.customer_id left join srx.warehouse on warehouse.id = customer.warehouse_id where warehouse.distributor_id = 2 and customer.created_by = 'LOAD SETUP' and general_setting.ship_to_id is not null")
        fetched_customer_settings = cursor.fetchall()
        print(len(fetched_customer_settings))
