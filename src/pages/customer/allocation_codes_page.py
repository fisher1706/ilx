...
class AllocationCodesPage(CustomerPortalPage):
    ...
    def add_allocation_code(self, allocation_code_body):
        ...
        self.set_slider(Locator.xpath_by_count(Locator.xpath_checkbox, 1), allocation_code_body.pop("isRequired"))
    ...
    def associate_subsite_with_allocation_code(self, subsite_name, subsite_number):
        ...
        self.select_checkbox(self.get_element_xpath_in_associate_table(row, 3) + Locator.xpath_checkbox)
        self.click_xpath(Locator.xpath_by_count(Locator.xpath_submit_button, 2))
    ...
    def check_associated(self, subsite_name, subsite_number, shipto_name_1, shipto_number_1, shipto_name_2, shipto_number_2, checked=None):
        ...
        for row_index in range(row, row + 3):
            if checked:
                self.checkbox_should_be(self.get_element_xpath_in_associate_table(row_index, 3) + Locator.xpath_checkbox, True)
            else:
                self.checkbox_should_be(self.get_element_xpath_in_associate_table(row_index, 3) + Locator.xpath_checkbox, False)
        self.click_xpath("//a[@href='/customer/allocation-codes']")
    ...
    def update_associated(self, subsite_name, subsite_number):
        ...
        for row_index in range(row + 1, row + 3):
            self.unselect_checkbox(self.get_element_xpath_in_associate_table(row_index, 3) + Locator.xpath_checkbox)
        self.click_xpath(Locator.xpath_by_count(Locator.xpath_submit_button, 2))
        self.click_xpath("//a[@href='/customer/allocation-codes']")
    ...
    def update_allocation_code(self, allocation_code_body):
        ...
        self.click_xpath("//a[@href='/customer/allocation-codes']")
        self.click_xpath(Locator.xpath_reload_button)
        self.element_should_have_text(Locator.xpath_last_role_row, allocation_code_body["name"])
